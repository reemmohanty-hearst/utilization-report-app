...
if uploaded:
    ...
    # ── Zero JIRA loggers — auto from Users sheet, or roster fallback ──
    st.divider()

    zero_df: pd.DataFrame = pd.DataFrame()
    zero_source = ""

    if tempo_users_df is not None:
        # PRIMARY: Tempo Users sheet — automatically extracted, no roster needed
        zero_df = find_zero_loggers_from_users_sheet(tempo_users_df)
        zero_source = f"Tempo Users sheet from {jira_filename_used}"
    elif not roster_df.empty:
        # FALLBACK: manual roster uploaded in sidebar
        ...
        zero_source = "Manual roster (sidebar upload)"

    zero_png = draw_zero_logger_chart(zero_df)

    if tempo_users_df is not None or not roster_df.empty:
        if zero_df.empty:
            st.success(f"✅ All **{len(tempo_users_df) if tempo_users_df is not None else len(roster_df)}** "
        ...
        st.info(
            "💡 **Zero-logger detection is automatic** — the app reads the `Users` sheet "
            "from your JIRA/Tempo file. If it wasn't found in your file, you can optionally "
            "upload a manual roster in the sidebar."
        )
