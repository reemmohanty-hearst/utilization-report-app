...
def tempo_team_name(team_id: str) -> str:
    resp = safe_request("GET", f"{TEMPO_BASE}/teams/{team_id}", auth=(), headers=_tempo_headers())
    return resp.json().get("name", f"Team {team_id}") or f"Team {team_id}"
...
def pull_week_worklogs_tempo(start: date, end: date) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for tid in TEMPO_TEAM_IDS:
        tname = tempo_team_name(tid)
        team_names[tid] = tname
        team_members[tid] = tempo_team_members(tid)
        ...
        for wl in logs:
            ...
            raw_logs.append(wl)
            fresh += 1
        print(f"[tempo] {tname} (id {tid}): {len(team_members[tid])} members, "
              f"{fresh} worklogs")
