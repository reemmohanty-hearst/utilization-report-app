...
def main() -> int:
    ...
    # Fail fast, with plain guidance, if the Jira/Confluence credential is bad
    try:
        me = safe_request("GET", f"{JIRA_BASE}/myself").json()
        print(f"[auth] Jira credential OK — authenticated as "
              f"{me.get('displayName', '?')} ({me.get('emailAddress', '')})")
    except Exception as e:
        print("[error] Jira/Confluence credential REJECTED "
              "(ATLASSIAN_EMAIL + ATLASSIAN_API_TOKEN pair).")
        print("        Fix: at id.atlassian.com -> Security -> API tokens, use the")
        print("        plain 'Create API token' button (NOT 'with scopes'), then")
        print("        UPDATE the ATLASSIAN_API_TOKEN secret in GitHub. Also confirm")
        print("        ATLASSIAN_EMAIL is exactly your Atlassian login email.")
