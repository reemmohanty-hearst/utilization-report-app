# app9.py  (app8 + unified Tempo file: JIRA + ServiceNow in one export)
# NEW IN app9:
#  - IT integrated ServiceNow ticket logging into Tempo, so both JIRA and
#    ServiceNow worklogs now arrive in a SINGLE Tempo Excel export.
#  - ServiceNow tickets are identified by their Issue Summary pattern:
#    they start with a fiscal-quarter prefix like "26Q1-2_HTS Network",
#    "26Q1-2_HTS Identity Team", etc.  Pattern: /^2\dQ\d/
#  - The app splits the single file into JIRA rows and SN rows automatically,
#    then builds separate pivots + a combined pivot, exactly as before.
#  - All hours from Tempo are already in hours (no seconds→hours conversion
#    needed for ServiceNow anymore — Tempo normalizes on ingest).
#  - Still supports uploading multiple files (e.g., if a week has two exports).
#  - All app8 functionality preserved: hardcoded roster, zero-logger detection,
#    Confluence publishing, department canonicalization, etc.
...
# =====================================================================
#  HARDCODED MASTER ROSTER - Jim Bazzano's Organization (58 people)
...
MASTER_ROSTER = [
    ...
    ("Jason Boulware", "Cloud Automation"),
    ("David Pickwell", "Cloud Automation"),
    ("Dev Patel", "Cloud Automation"),
    ("Bob Voelker", "CloudOps"),
    ("Jonathan Mims", "CloudOps"),
    ("Jane Lee", "CloudOps"),
    ("Steven Craig", "CCOE"),
...
def extract_tempo_users_sheet(file_bytes: bytes) -> Optional[pd.DataFrame]:
    """
    Read the 'Users' sheet from a Tempo/JIRA Excel export.
    This sheet lists EVERY person on the Tempo team for the period,
    including those who logged zero hours — which is exactly what we
    need to identify non-loggers without any separate roster file.

    Returns a tidy DataFrame with columns:
        Person, Logged Hours, Timesheet Status
    ...
    users_sheet = next(
        (s for s in xl.sheet_names if s.strip().lower() == "users"),
        None
    )
    ...
    # Name column
    name_col = next(
        (cols_lower[c] for c in ["full name", "name", "display name", "user"] if c in cols_lower),
        None
    )
    if name_col is None:
        return None
    ...
    # Logged hours column
    hours_col = next(
        (cols_lower[c] for c in ["logged hours", "hours logged", "hours", "logged_hours"] if c in cols_lower),
        None
    )

    # Progress % column (numeric: 0-100+)
    progress_col = cols_lower.get("progress", None)
    # Status text column — Tempo calls this "Period" (Approved / Waiting for approval / Ready to submit)
    status_col = cols_lower.get("period", progress_col)

    out = pd.DataFrame()
    out["Person"] = df[name_col].apply(canonicalize_person)
    out["Logged Hours"] = pd.to_numeric(df[hours_col], errors="coerce").fillna(0.0) if hours_col else 0.0
    out["Timesheet Status"] = df[status_col].fillna("Unknown").astype(str) if status_col else "Unknown"
    # Also keep the numeric progress % if available, for display
    if progress_col and progress_col != status_col:
        out["Progress %"] = pd.to_numeric(df[progress_col], errors="coerce").fillna(0.0)

    out = out[out["Person"].str.strip() != ""].reset_index(drop=True)
    return out


def find_zero_loggers_from_users_sheet(users_df: pd.DataFrame) -> pd.DataFrame:
    """
    Return rows from the Tempo Users sheet where Logged Hours == 0.
    Result columns: ['Person', 'Logged Hours', 'Timesheet Status']
    """
    if users_df is None or users_df.empty:
        return pd.DataFrame(columns=["Person", "Logged Hours", "Timesheet Status"])
    zero = users_df[users_df["Logged Hours"] <= 0].copy()
    return zero.sort_values("Person").reset_index(drop=True)


def load_roster(file_bytes: bytes, filename: str) -> pd.DataFrame:
    """
    Optional: load a manual roster CSV/XLSX (Name + Team columns) as a
    fallback when the JIRA file doesn't have a Users sheet.
    """
    try:
    ...
    name_col = next(
        (cols_lower[c] for c in ["full name", "name", "person", "display name", "employee"] if c in cols_lower),
        None
    )
    if name_col is None:
        raise ValueError(f"Need a Name column. Found: {list(df.columns)}")

    team_col = next(
        (cols_lower[c] for c in ["team", "department", "dept", "group", "assignment group",
                                  "tempo team", "department/team"] if c in cols_lower),
        None
    )
    if team_col is None:
        raise ValueError(f"Need a Team/Department column. Found: {list(df.columns)}")
...
st.sidebar.caption("Tip: set CONFLUENCE_USER in your Streamlit secrets or env vars to pre-fill the Email/User field.")

# Optional debug toggle
debug_mapping = st.sidebar.checkbox("Debug: show unmapped departments", value=False)

# ── Roster: Hardcoded by default, manual upload to override ──────
st.sidebar.divider()
st.sidebar.header("👥 Team Roster")

# Load hardcoded roster by default
roster_df: pd.DataFrame = get_hardcoded_roster()
st.sidebar.success(
    f"✅ **Hardcoded roster loaded:** {len(roster_df)} people across "
    f"{roster_df['Department/Team'].nunique()} teams"
)
st.sidebar.caption(
    "The master roster is hardcoded in the app (Jim Bazzano's 58-person org). "
    "To add/remove people, edit the `MASTER_ROSTER` list at the top of `app8.py`. "
    "**Coming soon:** Auto-sync from SharePoint (waiting on IT approval)."
)

# Optional: Allow manual override for testing
with st.sidebar.expander("🔧 Override with manual upload (testing only)"):
    roster_file = st.file_uploader(
        "Upload CSV/XLSX to override hardcoded roster",
        type=["csv", "xlsx"],
        key="roster_override",
    )
    if roster_file is not None:
        try:
            ...
            st.success(f"✅ Override loaded: {len(roster_df)} people")
        except Exception as e:
            st.error(f"Override error: {e}")
...
uploaded = st.file_uploader(
    "Drop your Tempo Excel file here (contains both JIRA and ServiceNow tickets).",
    type=["xlsx"],
    accept_multiple_files=True
)

# =====================================================================
#  ServiceNow ticket detection pattern
#  SN tickets imported into Tempo have Issue Summary starting with a
#  fiscal-quarter prefix like "26Q1-2_HTS ...", "25Q4-1_HTS ...", etc.
#  Pattern: 2-digit year + "Q" + quarter digit + dash + number + underscore
# =====================================================================
...
if uploaded:
    ...
    summary_lines = []
    tempo_users_df: Optional[pd.DataFrame] = None   # auto-detected from Tempo Users sheet
    jira_filename_used: str = ""
    ...
    with st.spinner("Processing files..."):
        for upl in uploaded:
            try:
                ...
                # All hours from Tempo are already in hours — no seconds conversion
                df["Hours"] = pd.to_numeric(df[work_col], errors="coerce").fillna(0.0).round(2)

                # ── Split into JIRA and ServiceNow rows ──
                if summary_col is not None:
                    sn_mask = df[summary_col].apply(is_servicenow_row)
                    df_jira = df[~sn_mask].copy()
                    df_sn = df[sn_mask].copy()
                else:
                    # No summary column found — treat everything as JIRA
                    df_jira = df.copy()
                    df_sn = pd.DataFrame()

                # Build JIRA pivot
                if not df_jira.empty:
                    pivot_j = build_pivot(df_jira, person_col, dept_col, "Hours")
                    jira_pivot = pd.concat([jira_pivot, pivot_j], ignore_index=True)

                # Build ServiceNow pivot
                if not df_sn.empty:
                    pivot_s = build_pivot(df_sn, person_col, dept_col, "Hours")
                    sn_pivot = pd.concat([sn_pivot, pivot_s], ignore_index=True)

                # ── Auto-extract the Tempo Users sheet for zero-logger detection ──
                if tempo_users_df is None:
                    extracted = extract_tempo_users_sheet(content)
                    if extracted is not None:
                        tempo_users_df = extracted
                        st.sidebar.success(
                            f"✅ Tempo Users sheet found: **{len(extracted)}** team members detected automatically."
                        )

                # ── Summary stats ──
                jira_count = len(df_jira)
                sn_count = len(df_sn)
                ...
                summary_lines.append(
                    ...
                    f"total_rows={len(df)}, jira_rows={jira_count}, servicenow_rows={sn_count} "
                    f"(SN detected by Issue Summary pattern); "
                    f"all hours from Tempo (already in hours); "
                    f"blanks={blanks}, zero_hours={zero}, outliers={outliers}."
                )

                st.success(
                    f"✅ **{upl.name}**: {len(df)} total rows → "
                    f"**{jira_count}** JIRA tickets + **{sn_count}** ServiceNow tickets"
                )
    ...
    # ── Zero JIRA loggers — use hardcoded roster first, Tempo Users as fallback ──
    st.divider()
    ...
    if not roster_df.empty:
        # PRIMARY: Use hardcoded master roster (Jim Bazzano's full 58-person org)
        raw_missing = find_zero_loggers_from_roster(roster_df, jira_pivot)
        # Reshape to match expected columns
        zero_df = raw_missing.rename(columns={"Department/Team": "Department/Team"})
        zero_df["Logged Hours"] = 0.0
        zero_source = f"Hardcoded master roster ({len(roster_df)} people)"
    elif tempo_users_df is not None:
        # FALLBACK: Tempo Users sheet (only people already in this Tempo team/project)
        zero_df = find_zero_loggers_from_users_sheet(tempo_users_df)
        zero_source = f"Tempo Users sheet from {jira_filename_used} ({len(tempo_users_df)} people)"

    zero_png = draw_zero_logger_chart(zero_df)

    if not roster_df.empty or tempo_users_df is not None:
        total_roster_count = len(roster_df) if not roster_df.empty else len(tempo_users_df)
        if zero_df.empty:
            st.success(f"✅ All **{total_roster_count}** "
                       f"team members logged JIRA/Tempo time this week — great job!")
        else:
            st.subheader(f"⚠️ No JIRA/Tempo Time Logged — {len(zero_df)} people")
            st.caption(
                f"These team members have **zero hours** in the Tempo report for this period. "
                f"Source: *{zero_source}*. "
                "May indicate forgot to log, were on leave, or timesheet not yet submitted."
            )

            # Summary table
            display_cols = [c for c in ["Person", "Timesheet Status", "Progress %", "Logged Hours", "Department/Team"]
                            if c in zero_df.columns]
            st.dataframe(
                zero_df[display_cols].reset_index(drop=True),
                use_container_width=True,
                hide_index=True,
            )
            st.image(zero_png, caption="No JIRA/Tempo Time Logged This Week")
    else:
        st.info(
            "💡 **Zero-logger detection is automatic** — the app uses the hardcoded master roster "
            "or reads the `Users` sheet from your JIRA/Tempo file."
        )
    ...
    if btn:
        if not (base_url and parent_page_id and user and token):
            ...
            try:
                ...
                # Upload zero-logger chart whenever we have Users sheet or roster data
                has_zero_data = tempo_users_df is not None or not roster_df.empty
                if has_zero_data:
                ...
                html = build_page_html(
                    ...
                    zero_df=zero_df if has_zero_data else None,
                    zero_png_name=zero_png_name if has_zero_data else None,
                    zero_source=zero_source if has_zero_data else "",
                )
